...
import appService from '../../appwrite/databaseService'
import { useNavigate } from 'react-router-dom'
import { useSelector } from 'react-redux'


function PostForm({post}) {
 const {register,habdleSubmit,watch,setValue,control,getValues}=useForm({
    defaultValues:{
        title : post?.title |"",
        slug : post?.slug || "",
...
const submit= async (data) =>{
    if(post)
    {
        data.image[0] ? appwriteService
                        ...
                        const dbPost =await appService.updatePost
                                          (
                                            post.$id,{
                                                ...data,
